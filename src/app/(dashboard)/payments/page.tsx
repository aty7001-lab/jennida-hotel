...
export default async function PaymentsPage({
  ...
  searchParams: Promise<{ q?: string; status?: string; start?: string; end?: string }>;
}) {
  ...
  const allPayments = await getPayments(activeBranchId, startDate, endDate);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <a
          href={csvUri}
          download="payments_export.csv"
          className="bg-indigo-600 text-white px-4 py-2 rounded-md font-medium text-sm shadow-sm hover:bg-indigo-700 transition-colors flex items-center justify-center gap-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
        >
          <Download size={16} />
      ...
      <div className="bg-white rounded-md shadow-sm border border-slate-200 overflow-hidden">
        ...
        <form className="p-4 border-b border-slate-200 flex flex-col sm:flex-row items-center gap-4 bg-slate-50/30">
          ...
          <select
            ...
            className="w-full sm:w-40 border border-slate-300 rounded-md px-3 py-2 text-sm bg-white text-slate-700 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
          >
        ...
        <div className="px-6 py-4 border-t border-slate-200 bg-slate-50/50">
          <span className="text-sm text-slate-500">
            ສະແດງ <span className="font-medium text-slate-900">{payments.length}</span> ຈາກ{" "}
            <span className="font-medium text-slate-900">{allPayments.length}</span> ລາຍການ
          </span>
        </div>
      </div>
    </div>
